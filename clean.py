...
def print_summary(count: int, size_mb: float, item_type: str = "items", dry_run: bool = True):
    ...
    if dry_run:
        print(f"💡 DRY-RUN: Sử dụng --execute để thực sự xóa")
    print(f"{'='*60}\n")
...
def clean_cache(*, force: bool = False, older_than_days: int = 30, dry_run: bool = True) -> tuple[int, float]:
    ...
    for file_path in cache_dir.glob("*.csv"):
        ...
        should_remove = force or age_days > older_than_days
...
Ví dụ:
  ...
  python clean.py --reports --keep 3 # Giữ lại 3 báo cáo
    ...
    parser.add_argument('--all', '-a', action='store_true', help='Xóa tất cả')
...
def main():
    ...
    do_all = args.all or not any([args.cache, args.data_cache, args.reports, args.checkpoints])
    ...
    # Cache
    if do_all or args.cache or args.data_cache:
        force = args.cache or (do_all and not dry_run)
