...
Usage:
    python clean.py                    # Xem trước (dry-run)
    python clean.py --execute          # Thực sự xóa
    python clean.py --cache --days 7  # Xóa cache > 7 ngày
    python clean.py --reports --keep 3 # Giữ lại 3 báo cáo mới nhất
"""
...
def clean_cache(*, force: bool = False, older_than_days: int = 30, dry_run: bool = True) -> tuple[int, float]:
    """Xóa cache dữ liệu"""
    cache_dir = BASE / "step1_data" / "cache"
...
def clean_checkpoints(*, keep_best: bool = True, dry_run: bool = True) -> tuple[int, float]:
    """Xóa checkpoints"""
    checkpoint_dir = BASE / "reports" / "checkpoints"
...
Ví dụ:
  ...
  python clean.py --execute          # Thực sự xóa
  python clean.py --cache --days 7   # Xóa cache > 7 ngày
  python clean.py --reports --keep 3 # Giữ lại 3 báo cáo
        """
    )
    ...
    # Cache flags (giữ tương thích với README cũ)
    parser.add_argument('--cache', action='store_true', help='Xóa TẤT CẢ cache (tương đương --data-cache-force)')
    parser.add_argument('--data-cache', action='store_true', help='Xóa cache cũ (> --days)')
    parser.add_argument('--data-cache-force', action='store_true', help='Xóa TẤT CẢ cache dữ liệu (không quan tâm --days)')
    parser.add_argument('--days', type=int, default=30, help='Số ngày cache cũ (mặc định: 30)')
    
    # Reports flags (giữ tương thích với README cũ)
    ...
    parser.add_argument('--keep-reports', type=int, help='Alias của --keep (tương thích docs cũ)')
...
def main():
    ...
    # Nếu user không chỉ định gì, mặc định chạy "dọn tất cả loại" ở chế độ an toàn
    ...
    if do_all or args.cache or args.data_cache or args.data_cache_force:
        # do_all => chỉ xóa cache cũ theo --days (mặc định 30 ngày)
